...
pub mod resolve {
    ...
    flowey_request! {
        pub enum Request {
            LastRelease(WriteVar<ReleaseOutput>),
        }
    }
    ...
    impl SimpleFlowNode for Node {
        ...
        fn imports(ctx: &mut ImportCtx<'_>) {
            ...
            ctx.import::<flowey_lib_common::gh_latest_release_branch::Node>();
        }

        fn process_request(request: Self::Request, ctx: &mut NodeCtx<'_>) -> anyhow::Result<()> {
            match request {
                Request::LastRelease(output) => {
                    // First, get the latest release branch
                    let latest_release_branch =
                        ctx.reqv(|v| flowey_lib_common::gh_latest_release_branch::Request {
                            latest_release_branch: v,
                        });

                    // Use the latest release branch to get the workflow ID
                    let run_id =
                        ctx.reqv(
