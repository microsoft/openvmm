...
impl SimpleFlowNode for Node {
    ...
    fn process_request(request: Self::Request, ctx: &mut NodeCtx<'_>) -> anyhow::Result<()> {
        ...
        let client_id = ctx.get_gh_context_var().global(client_id);
        let tenant_id = ctx.get_gh_context_var().global(tenant_id);
        let subscription_id = ctx.get_gh_context_var().global(subscription_id);
