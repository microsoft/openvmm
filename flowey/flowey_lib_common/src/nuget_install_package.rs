...
"#
                    ...
                    let feed_endpoints_json = if matches!(rt.backend(), FlowBackend::Local) {
                        get_feed_endpoints_json(parsed.feed_urls)?
                    } else {
                        None
                    };
...
fn get_feed_endpoints_json(feed_urls: Vec<String>) -> anyhow::Result<Option<String>> {
    ...
    let session_token_body_str = session_token_body.to_string();
    let session_response = xshell::cmd!(
        sh,
        "curl -s --fail -X POST https://app.vssps.visualstudio.com/_apis/token/sessiontokens?api-version=5.0-preview.1 -H Content-Type:application/json -K - -d {session_token_body_str}"
    )
