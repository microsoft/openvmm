...
pub enum OpenvmmPipelinesCi {
    ...
    /// Build, validate, and draft an OpenVMM release.
    OpenvmmRelease(openvmm_release::OpenvmmReleaseCli),
}
    ...
    fn into_pipeline(self, pipeline_hint: PipelineBackendHint) -> anyhow::Result<Pipeline> {
        match self {
            ...
            OpenvmmPipelines::Ci(cmd) => match cmd {
                ...
                OpenvmmPipelinesCi::OpenvmmRelease(cmd) => cmd.into_pipeline(pipeline_hint),
            },
