    ...
    for config in args.configs {
        ...
        let (cfg_conditions, artifacts, petri_vm_config) = match (specific_vmm, config.vmm) {
            ...
            | (None, Some(Vmm::HyperV)) => (
                quote!(#[cfg(windows)]),
                quote!(::petri::PetriVmArtifacts::new::<::petri::hyperv::HyperVPetriBackend>),
                quote!(::petri::PetriVmBuilder::new::<::petri::hyperv::HyperVPetriBackend>),
            ),
            ...
            | (None, Some(Vmm::OpenVmm)) => (
                quote!(),
                quote!(::petri::PetriVmArtifacts::new::<::petri::openvmm::OpenVmmPetriBackend>),
                quote!(::petri::PetriVmBuilder::new::<::petri::openvmm::OpenVmmPetriBackend>),
            ),
            (None, None) => return Err(Error::new(config.span, "vmm must be specified")),
            _ => return Err(Error::new(config.span, "vmm mismatch")),
        };

        let petri_vm_config = quote!(#petri_vm_config(&params, artifacts, &driver)?);
