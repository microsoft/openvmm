...
#[openvmm_test(openvmm_openhcl_uefi_x64(vhd(ubuntu_2204_server_x64))[LATEST_STANDARD_X64, VMGS_WITH_16K_TPM])]
async fn tpm_servicing(
    config: PetriVmBuilder<OpenVmmPetriBackend>,
    (igvm_file, vmgs_file): (
    ...
    let inspect_after = vm.inspect("vm/tpm/worker/nvram_size", None, None).await?;
