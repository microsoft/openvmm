...
impl MshvProcessor<'_> {
    ...
    ) -> Result<(), VpHaltReason<mshv_ioctls::MshvError>> {
    ...
    fn emulation_cache(&self) -> Result<MshvEmuCache, mshv_ioctls::MshvError> {
...
impl EmulatorSupport for MshvEmulationState<'_> {
    type Error = mshv_ioctls::MshvError;
    ...
    fn instruction_bytes(&self) -> &[u8] {
        match HvMessageType(self.message.header.message_type) {
            ...
            | HvMessageType::HvMessageTypeUnacceptedGpa => {
                ...
                unsafe {
                    let info = (&raw const self.message.u.payload)
                        as *const hv_x64_memory_intercept_message;
            ...
            HvMessageType::HvMessageTypeX64IoPortIntercept => {
                ...
                unsafe {
                    let info = (&raw const self.message.u.payload)
                        as *const hv_x64_io_port_intercept_message;
                    let instruction_bytes = &raw const (*info).instruction_bytes;
                    let instruction_byte_count =
                        std::ptr::read_unaligned(&raw const (*info).instruction_byte_count);
                    std::slice::from_raw_parts(
        ...
        self.partition
            ...
            .any(|range| {
                (range.guest_pfn..range.guest_pfn + range.size).contains(&gpa)
                    && (!write || range.flags & hvdef::HvInterceptAccessType::WRITE.0 == hvdef::HvInterceptAccessType::WRITE.0)
            })
    }
...
impl virt::PartitionMemoryMap for MshvPartitionInner {
    unsafe fn map_range(
        ...
        let mut flags = 0;
        if writable {
            flags |= hvdef::HvInterceptAccessType::WRITE.0;
        }
        if exec {
            flags |= hvdef::HvInterceptAccessType::EXECUTE.0;
        }
...
impl virt::Processor for MshvProcessor<'_> {
    type Error = Error;
    type RunVpError = mshv_ioctls::MshvError;
    type StateAccess<'a>
        = &'a mut Self
    where
        Self: 'a;
    ...
    async fn run_vp(
        ...
        stop: StopVp<'_>,
        dev: &impl CpuIo,
    ) -> Result<Infallible, VpHaltReason<mshv_ioctls::MshvError>> {
