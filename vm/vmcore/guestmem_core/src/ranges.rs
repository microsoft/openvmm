...
impl<'a> PagedRange<'a> {
    ...
    pub fn reader<M: GuestMemoryIo + ?Sized>(self, mem: &'a M) -> PagedRangeReader<'a, M> {
        PagedRangeReader { range: self, mem }
    }

    /// Returns a [`MemoryWrite`] implementation.
    pub fn writer<M: GuestMemoryIo + ?Sized>(self, mem: &'a M) -> PagedRangeWriter<'a, M> {
...
pub struct PagedRangeReader<'a, M: ?Sized> {
    range: PagedRange<'a>,
    mem: &'a M,
}

impl<M: GuestMemoryIo + ?Sized> MemoryRead for PagedRangeReader<'_, M> {
...
pub struct PagedRangeWriter<'a, M: ?Sized> {
    range: PagedRange<'a>,
    mem: &'a M,
}

impl<M: GuestMemoryIo + ?Sized> MemoryWrite for PagedRangeWriter<'_, M> {
...
impl<'a, T: Iterator<Item = PagedRange<'a>>> PagedRanges<'a, T> {
    ...
    pub fn reader<M: GuestMemoryIo + ?Sized>(self, mem: &'a M) -> PagedRangesReader<'a, T, M> {
        PagedRangesReader { views: self, mem }
    }

    /// Returns a [`MemoryWrite`] implementation for the ranges.
    pub fn writer<M: GuestMemoryIo + ?Sized>(self, mem: &'a M) -> PagedRangesWriter<'a, T, M> {
...
#[derive(Debug, Clone)]
pub struct PagedRangesReader<'a, T, M: ?Sized> {
    views: PagedRanges<'a, T>,
    mem: &'a M,
}

impl<'a, T, M: ?Sized> PagedRangesReader<'a, T, M> {
...
impl<'a, T: Iterator<Item = PagedRange<'a>>, M: GuestMemoryIo + ?Sized> MemoryRead
    for PagedRangesReader<'a, T, M>
{
...
#[derive(Debug)]
pub struct PagedRangesWriter<'a, T, M: ?Sized> {
    views: PagedRanges<'a, T>,
    mem: &'a M,
}

impl<'a, T, M: ?Sized> PagedRangesWriter<'a, T, M> {
    /// Returns the inner ranges.
    pub fn into_inner(self) -> PagedRanges<'a, T> {
        self.views
    }
...
impl<'a, T: Iterator<Item = PagedRange<'a>>, M: GuestMemoryIo + ?Sized> MemoryWrite
    for PagedRangesWriter<'a, T, M>
{
