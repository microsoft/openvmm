...
impl<T: DeviceBacking> ManaDevice<T> {
    ...
    /// Test-only variant of [`new`](Self::new) that allows injecting an
    /// expected [`ManaQueryDeviceCfgResp`] in place of the hardware response
    /// returned by `query_dev_config`.
    ///
    /// When `expect_dev_config` is `Some(cfg)`, `cfg` is stored as the device
    /// configuration instead of what the emulator returned, letting tests
    /// observe a specific value (e.g. a non-zero link speed) through
    /// `dev_config()`, `vport.link_speed_bps()`, and `endpoint.link_speed()`.
    /// Pass `None` for behaviour identical to `new`.
    ///
    /// # Maintenance note
    ///
    /// This function is a near-copy of [`new`](Self::new) and differs only in
    /// the single `query_dev_config_test` call. If `new` ever changes (new
    /// parameters, different initialization steps, etc.) **this function must
    /// be updated to match**.
    ...
    pub async fn new_test(
        ...
        max_queues_per_vport: u16,
        mana_state: Option<&ManaDeviceSavedState>,
        expect_dev_config: Option<ManaQueryDeviceCfgResp>,
    ) -> anyhow::Result<Self> {
        let mut gdma = if let Some(mana_state) = mana_state {
            let memory = device.dma_client().attach_pending_buffers()?;
            let gdma_memory = memory
        ...
        let mut bnic = BnicDriver::new(&mut gdma, dev_id);
        // Use query_dev_config_test so that tests can substitute an expected
        // response body (e.g. a non-zero link speed) for the hardware value.
        let dev_config = bnic.query_dev_config_test(expect_dev_config).await?;
        tracing::info!(mana_dev_config = ?dev_config);
        let num_queues_needed = dev_config.max_num_vports as u32 * max_queues_per_vport as u32;
        gdma.check_vf_resources(num_vps, num_queues_needed);
