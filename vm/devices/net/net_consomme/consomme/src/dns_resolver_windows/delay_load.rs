...
// DnsQueryEx APIs (Windows 8+)
define_dns_api!(get_dns_query_ex, "DnsQueryEx");
define_dns_api!(get_dns_cancel_query, "DnsCancelQuery");
...
/// Check if DnsQueryEx APIs are available (Windows 8+).
pub fn is_dns_query_ex_supported() -> bool {
    get_dns_query_ex().is_ok() && get_dns_cancel_query().is_ok()
}
