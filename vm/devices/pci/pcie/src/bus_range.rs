...
//!
//! An [`AssignedBusRange`] holds the bus range `(secondary_bus, subordinate_bus)`
//! assigned to the PCIe port that owns a device. It is updated by the
//! PCIe port when the guest assigns bus numbers.
//!
//! Consumers (ITS wrappers, SMMU) compose a full device identity from the
//! bus range plus the device's BDF.

use std::sync::Arc;
use std::sync::atomic::AtomicU16;
use std::sync::atomic::Ordering;

/// Shared PCI device identity.
///
/// Stores a packed `(secondary_bus, subordinate_bus)` as an atomic u16,
/// updated when the PCIe port's bus numbers change.
///
...
impl AssignedBusRange {
    /// Creates a new device ID initialized to zero.
    pub fn new() -> Self {
