...
//!
//! (Mapping buffers into the IOAS via `IOMMU_IOAS_MAP` would not work here
//! anyway: the page pool mmaps `/dev/mshv_vtl_low`, a `VM_MIXEDMAP` mapping
//! backed by ZONE_DEVICE (`MEMORY_DEVICE_GENERIC`) memory. `IOMMU_IOAS_MAP`
//! long-term pins the user pages, and ZONE_DEVICE device memory is not
//! long-term pinnable, so the pin fails.)
...
impl IommufdIoas {
    ...
    /// buffers are a `VM_MIXEDMAP` mapping of ZONE_DEVICE memory, which
    /// `IOMMU_IOAS_MAP` cannot long-term pin).
