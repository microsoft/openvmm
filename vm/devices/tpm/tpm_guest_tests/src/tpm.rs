...
impl Tpm {
    ...
    pub fn open_default() -> io::Result<Self> {
        #[cfg(unix)]
        {
            UnixTpm::open_default().map(|u| Tpm {
                inner: Inner::Unix(u),
            })
        }
        #[cfg(windows)]
        {
            WindowsTpm::open().map(|w| Tpm {
...
#[cfg(unix)]
impl UnixTpm {
    fn open_default() -> io::Result<Self> {
        ...
        Err(last_err.unwrap_or_else(|| io::Error::new(io::ErrorKind::NotFound, "No TPM device")))
    }
...
mod win_ffi {
    ...
    unsafe extern "system" {
        pub fn Tbsi_Context_Create(params: *const TBS_CONTEXT_PARAMS2, handle: *mut u32) -> u32;
