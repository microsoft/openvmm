...
impl<T: DeviceBacking> NvmeDriver<T> {
    ...
    async fn enable(&mut self, requested_io_queue_count: u16) -> anyhow::Result<()> {
        ...
        let qsize = {
            let io_cqsize = QueuePair::MAX_CQ_ENTRIES.min(worker.registers.cap.mqes_z().saturated_add(1));
            let io_sqsize = QueuePair::MAX_SQ_ENTRIES.min(worker.registers.cap.mqes_z().saturated_add(1));
