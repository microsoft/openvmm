...
mod tests {
    ...
    //   name("TestVar" as UCS-2 LE, no NUL)
    //   || vendor(16 zero bytes)
    //   || attr(0x27 LE u32)
    //   || timestamp(16 zero bytes)
    //   || var_data(b"hello uefi")
    const TEST_CERT_DER: &[u8] = include_bytes!("test_data/auth_var_cert.der");
    const TEST_PKCS7_DER: &[u8] = include_bytes!("test_data/auth_var_pkcs7.der");
    const TEST_NAME: &str = "TestVar";
    const TEST_ATTR: u32 = 0x27;
    const TEST_VAR_DATA: &[u8] = b"hello uefi";

    fn signature_list_from_cert(cert_der: &[u8]) -> Vec<u8> {
        let list =
            SignatureList::X509(SignatureData::new_x509(Guid::ZERO, Cow::Borrowed(cert_der)));
        let mut buf = Vec::new();
        list.extend_as_spec_signature_list(&mut buf);
        buf
    }

    fn make_parsed_auth_var<'a>(name: &'a Ucs2LeVec, var_data: &'a [u8]) -> ParsedAuthVar<'a> {
        ParsedAuthVar {
            name: name.as_ref(),
            vendor: Guid::ZERO,
            attr: TEST_ATTR,
            timestamp: EFI_TIME::ZEROED,
            pkcs7_data: TEST_PKCS7_DER,
            var_data,
        }
    }

    /// Valid PKCS#7 signature over the matching content should verify.
    #[test]
    fn authenticate_variable_success() {
        let signature_lists = signature_list_from_cert(TEST_CERT_DER);
        let name = Ucs2LeVec::from(TEST_NAME);
        let var = make_parsed_auth_var(&name, TEST_VAR_DATA);

        let result = authenticate_variable(&signature_lists, var).expect("verification errored");
        assert!(result, "expected valid PKCS#7 signature to verify");
    }

    /// Tampered content should fail verification (but not error out).
    #[test]
    fn authenticate_variable_tampered_content() {
        let signature_lists = signature_list_from_cert(TEST_CERT_DER);
        let name = Ucs2LeVec::from(TEST_NAME);
        let tampered = b"HELLO uefi";
        let var = make_parsed_auth_var(&name, tampered);

        let result = authenticate_variable(&signature_lists, var).expect("verification errored");
        assert!(!result, "expected tampered content to fail verification");
    }

    /// Empty signature list (no trusted certs) should fail verification.
    #[test]
    fn authenticate_variable_no_trusted_cert() {
        let name = Ucs2LeVec::from(TEST_NAME);
        let var = make_parsed_auth_var(&name, TEST_VAR_DATA);

        let result = authenticate_variable(&[], var).expect("verification errored");
        assert!(
            !result,
            "expected verification to fail with empty trust store"
        );
    }
}
