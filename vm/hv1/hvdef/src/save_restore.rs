...
///
/// Stored as the `RamMemoryBlock%d` array values in the VMRS key-value store.
/// Each instance describes a contiguous guest physical memory range and maps
/// it to the corresponding `RamBlock%I64u` data keys.
#[repr(C)]
#[derive(Copy, Clone, Debug, IntoBytes, Immutable, KnownLayout, FromBytes)]
...
static_assertions::const_assert_eq!(size_of::<MemoryBlockSaveStruct>(), 48);
