...
pub fn tpmt_sensitive_marshal(
    ...
    tracing::trace!("Marshaling sensitive_type: {} bytes = {:02X?}", 
             sensitive_type_bytes.len(), sensitive_type_bytes);
    ...
    tracing::trace!("Marshaling auth_value: {} bytes = {:02X?}", 
             auth_value_bytes.len(), 
             if auth_value_bytes.len() <= 8 { &auth_value_bytes[..] } else { &auth_value_bytes[..8] });
    ...
    tracing::trace!("Marshaling seed_value: {} bytes = {:02X?}", 
             seed_value_bytes.len(),
             if seed_value_bytes.len() <= 8 { &seed_value_bytes[..] } else { &seed_value_bytes[..8] });
    ...
    tracing::trace!("Marshaling sensitive: {} bytes = {:02X?}",
             sensitive_bytes.len(),
             if sensitive_bytes.len() <= 8 { &sensitive_bytes[..] } else { &sensitive_bytes[..8] });
