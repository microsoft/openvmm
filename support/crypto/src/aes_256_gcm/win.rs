...
impl KeyBlob {
    fn new(key: &[u8]) -> Result<Self, Aes256GcmError> {
        let key_a: [u8; KEY_SIZE] = key.try_into().map_err(|_| {
            Aes256GcmError(crate::BackendError(
    ...
    unsafe {
        let alg_handle = {
            BCryptOpenAlgorithmProvider(&mut alg, BCRYPT_AES_ALGORITHM, None, Default::default())
                .ok()
                .map_err(|e| {
                    Aes256GcmError(crate::BackendError(e, "opening algorithm provider"))
                })?;
            AlgWrapper { alg }
        };
        ...
        .ok()
        .map_err(|e| Aes256GcmError(crate::BackendError(e, "setting GCM Property")))?;
        ...
        .ok()
        .map_err(|e| Aes256GcmError(crate::BackendError(e, "importing key")))?;
