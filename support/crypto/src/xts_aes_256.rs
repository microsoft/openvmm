...
#[derive(Debug, Error)]
#[error(transparent)]
pub struct XtsAes256Error(sys::XtsAes256ErrorInner);
...
mod sys {
    ...
    pub type XtsAes256ErrorInner = openssl::error::ErrorStack;

    pub fn xts_aes_256(
        key: &[u8],
        _data_unit_size: u32,
    ) -> Result<XtsAes256Inner, XtsAes256ErrorInner> {
    ...
    impl XtsAes256Inner {
        pub fn ctx(&self, enc: bool) -> Result<XtsAes256CtxInner<'_>, XtsAes256ErrorInner> {
    ...
    impl XtsAes256CtxInner<'_> {
        pub fn cipher(&mut self, iv: &[u8], data: &mut [u8]) -> Result<(), XtsAes256ErrorInner> {
            if self.enc {
...
mod sys {
    ...
    #[derive(Debug, Error)]
    #[error("{op} failed")]
    pub struct XtsAes256ErrorInner {
        op: &'static str,
        #[source]
        err: std::io::Error,
    }
    ...
    impl XtsAes256CtxInner<'_> {
        pub fn cipher(&self, tweak: &[u8; 16], data: &mut [u8]) -> Result<(), XtsAes256ErrorInner> {
            let mut iv = u64::try_from(u128::from_le_bytes(*tweak))
                .map_err(|_| XtsAes256ErrorInner {
                    op: "convert tweak",
    ...
    fn bcrypt_result(op: &'static str, status: NTSTATUS) -> Result<(), XtsAes256ErrorInner> {
        if status.is_ok() {
            Ok(())
        } else {
            // SAFETY: no preconditions for this call.
            let err = unsafe { RtlNtStatusToDosError(status) };
            Err(XtsAes256ErrorInner {
                op,
    ...
    impl Key {
        fn encrypt(&self, iv: &mut [u8], data: &mut [u8]) -> Result<(), XtsAes256ErrorInner> {
        ...
        fn decrypt(&self, iv: &mut [u8], data: &mut [u8]) -> Result<(), XtsAes256ErrorInner> {
            // TODO: fix windows crate to allow aliased input and output, as
            // allowed by the API.
            let input = data.to_vec();
            let mut n = 0;
