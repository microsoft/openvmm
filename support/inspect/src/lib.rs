...
/// An error indicating that a `ValueKind` could not be converted to the
/// requested type using [`TryFrom<ValueKind>::try_from`].
#[cfg(feature = "initiate")]
pub struct WrongValueKindForType;

#[cfg(feature = "initiate")]
impl TryFrom<ValueKind> for bool {
    type Error = WrongValueKindForType;
    fn try_from(v: ValueKind) -> Result<Self, Self::Error> {
        match v {
            ValueKind::Bool(b) => Ok(b),
            _ => Err(WrongValueKindForType),
        }
    }
}

#[cfg(feature = "initiate")]
impl TryFrom<ValueKind> for f32 {
    type Error = WrongValueKindForType;
    fn try_from(v: ValueKind) -> Result<Self, Self::Error> {
        match v {
...
#[cfg(feature = "initiate")]
impl TryFrom<ValueKind> for Vec<u8> {
    type Error = WrongValueKindForType;
    fn try_from(v: ValueKind) -> Result<Self, Self::Error> {
        match v {
...
macro_rules! try_from_as {
    ($x:ident, $to:ty, $($ty:ty),* $(,)?) => {
        $(
            impl TryFrom<ValueKind> for $ty {
                type Error = WrongValueKindForType;
                fn try_from(v: ValueKind) -> Result<Self, Self::Error> {
                    match v {
                        ValueKind::$x(n) => Ok(n as $ty),
                        _ => Err(WrongValueKindForType),
                    }
                }
            }
...
#[cfg(feature = "initiate")]
try_from_as!(Signed, i64, i8, i16, i32, i64, isize);

#[cfg(feature = "initiate")]
try_from_as!(Unsigned, u64, u8, u16, u32, u64, usize);
