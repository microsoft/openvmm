...
impl OsVersionInfo {
    pub fn new() -> Self {
        ...
        let major_minor = {
            ...
            // SAFETY: utsname will be initialized by uname, so we zero it first
            unsafe { std::ptr::write_bytes(utsname.as_mut_ptr(), 0, 1) };
            let mut utsname = unsafe { utsname.assume_init() };
