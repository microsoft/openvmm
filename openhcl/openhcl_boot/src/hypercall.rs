...
const MAX_INPUT_ELEMENTS: usize = (HV_PAGE_SIZE as usize - PIN_REQUEST_HEADER_SIZE) / size_of::<u64>();
...
impl HvCall {
    ...
    fn to_hv_gva_range_array(memory_range: &MemoryRange) -> ArrayVec<HvGpaRange, MAX_INPUT_ELEMENTS> {
    ...
    pub fn pin_gpa_range(&mut self, memory_range: MemoryRange) -> Result<(), hvdef::HvError> {
       ...
       // Calculate the total number of pages per request
        ...
        let header = hvdef::hypercall::PinUnpinGpaPageRangesHeader {
            reserved: 0,
        };
        ...
        while remaining_size_bytes > 0 {
            ...
            let gva_ranges: ArrayVec<HvGpaRange, MAX_INPUT_ELEMENTS> = Self::to_hv_gva_range_array(&sub_range);
