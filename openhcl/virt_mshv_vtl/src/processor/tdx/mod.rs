...
impl UhProcessor<'_, TdxBacked> {
    ...
    fn try_poll_apic(&mut self, vtl: GuestVtl, scan_irr: bool) -> Result<bool, UhRunVpError> {
        ...
        super::apic::poll_apic_core(
            &mut scan,
            |this| {
                this.processor.backing.cvm.lapics[vtl]
                    .lapic
                    .scan(&mut this.processor.vmtime, scan_irr)
            },
            |this| this.processor.runner.proxy_irr(),
            |this| &mut this.processor.backing.cvm.lapics[vtl],
            |this| *this.processor.inner.hcvm_vtl1_enabled.lock(),
            vtl,
        )?;
...
struct TdxApicScanner<'a, 'b> {
    processor: &'a mut UhProcessor<'b, TdxBacked>,
    processor_controls: ProcessorControls,
    tpr_threshold: u8,
}

impl ApicBacking for TdxApicScanner<'_, '_> {
    fn handle_interrupt(&mut self, vtl: GuestVtl, vector: u8) -> Result<(), UhRunVpError> {
        ...
        if self.processor.backing.cvm.lapics[vtl].activity == MpState::Idle {
            self.processor.backing.cvm.lapics[vtl].activity = MpState::Running;
        }
        ...
        if self.processor.backing.vtls[vtl]
            .interruption_information
            .valid()
            && self.processor.backing.vtls[vtl]
                .interruption_information
                .interruption_type()
                != INTERRUPT_TYPE_EXTERNAL
        {
        ...
        let rflags = RFlags::from(self.processor.runner.tdx_enter_guest_state().rflags);
        ...
        let apic: &ApicPage = zerocopy::transmute_ref!(self.processor.runner.tdx_apic_page());
        if (apic.tpr.value as u8 >> 4) >= priority {
            self.tpr_threshold = priority;
            return Ok(());
        }

        self.processor.backing.vtls[vtl].interruption_information = InterruptionInformation::new()
            .with_valid(true)
            .with_vector(vector)
    ...
    fn handle_init(&mut self, vtl: GuestVtl) -> Result<(), UhRunVpError> {
        let vp_info = self.processor.inner.vp_info;
        {
            let mut access = self.processor.access_state(vtl.into());
    ...
    fn handle_sipi(&mut self, vtl: GuestVtl, vector: u8) -> Result<(), UhRunVpError> {
        if self.processor.backing.cvm.lapics[vtl].activity == MpState::WaitForSipi {
            ...
            self.processor
                ...
                .unwrap();
            self.processor.runner.tdx_enter_guest_state_mut().rip = 0;
            self.processor.backing.cvm.lapics[vtl].activity = MpState::Running;
        }
        Ok(())
    }
