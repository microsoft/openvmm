        ...
        loop {
            ...
            match event {
                ...
                Event::Halt(reason) => {
                    ...
                    // On a guest crash, write a .vmrs dump before applying the
                    // configured crash action (which may reset or exit the VM).
                    if matches!(reason, HaltReason::TripleFault { .. }) {
                        if let Some(path) = self.crash_dump_path.clone() {
                            tracing::info!(
                                path = %path.display(),
                                "dumping VM state on guest crash"
                            );
                            match self.handle_dump_state(&path).await {
                                ...
                                Err(err) => {
                                    tracing::error!(
                                        ...
                                        path = %path.display(),
                                        "failed to dump VM state on guest crash"
                                    );
                                }
                            }
                        }
