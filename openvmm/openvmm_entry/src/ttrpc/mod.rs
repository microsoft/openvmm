...
use platform::BoundListener;
use platform::FdRegistry;
pub use platform::Listener;
pub use platform::listener_from_path;
...
fn bind_unix_listener(path: &std::path::Path) -> anyhow::Result<UnixListener> {
...
mod dispatch {
    ...
    pub(super) async fn serve_by_protocol<T>(
        server: &mesh_rpc::Server,
        conn: T,
        first_byte: u8,
        transport: ResolvedTransport,
        shutdown_policy: mesh_rpc::server::ShutdownPolicy,
    ) -> anyhow::Result<()>
    where
        T: AsyncRead + AsyncWrite + Unpin,
    {
        #[cfg(not(feature = "grpc"))]
        let _ = shutdown_policy;

        match first_byte {
            #[cfg(feature = "ttrpc")]
            0x00 if transport.allows_ttrpc() => server.serve_connection(conn).await,
            #[cfg(feature = "grpc")]
            b'P' if transport.allows_grpc() => {
                server.serve_connection_grpc(conn, shutdown_policy).await
            }
            byte => {
    ...
    async fn serve(
        ...
        super::platform::serve_socket(server, conn, first_byte, transport, registry).await
    }
...
impl Worker for TtrpcWorker {
    ...
    fn new(parameters: Self::Parameters) -> anyhow::Result<Self> {
        Ok(Self {
            listener: BoundListener::new(parameters.listener)?,
...
impl VmService {
    async fn run(
        ...
        let server_task = self.driver.spawn("ttrpc-server", {
            ...
            async move {
                let r = listener
                    .run(&server, &driver, cancel_recv, transport, registry)
                    .await;
                match &r {
