...
Standalone OpenVMM releases derive their three-component product version from
an exact `openvmm-vMAJOR.MINOR.PATCH` tag. Untagged builds use a development
identity based on the source revision. OpenVMM versions remain independent from
the OpenHCL release branches described below and from CI build numbers.
