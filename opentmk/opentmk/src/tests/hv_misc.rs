...
pub fn exec(_opt: Option<()>, ctx: Arc<RefCell<dyn TestCtxTrait>>) {
    log::info!("ctx ptr: {:p}", &ctx as *const _);
    
    let mut cpy = ctx.clone();
    ...
    ctx.start_on_vp(
        VpExecutor::new(0, Vtl::Vtl1).command(move |ctx: &mut dyn TestCtxTrait| {
            ...
            ctx.set_interupt_idx(0x30, || {
                ...
                // let mut hv_test_ctx: HvTestCtx = HvTestCtx::new();
                // hv_test_ctx.init();

                // let c = hv_test_ctx.get_register(HvAllArchRegisterName::VsmVpStatus.0);

                // let cp: HvRegisterVsmVpStatus = HvRegisterVsmVpStatus::from_bits(c as u64);
    ...
    log::info!("ctx ptr: {:p}", &ctx as *const _);
    log::info!("_opt ptr: {:p}", &_opt as *const _);
    let mut l = 0u64;
    ...
    let mut ctx = cpy.borrow_mut();
    // let mut ctx = cpy.borrow_mut();
    log::info!("ctx ptr: {:p}", &ctx as *const _);
    log::info!("opt ptr: {:p}", &_opt as *const _);
