...
fn memdiff_disk(path: &Path) -> anyhow::Result<Resource<DiskHandleKind>> {
    ...
    Ok(LayeredDiskHandle {
        layers: vec![
            RamDiskLayerHandle { len: None, sector_size: None }.into_resource().into(),
...
fn petri_disk_to_openvmm(disk: &Disk) -> anyhow::Result<Resource<DiskHandleKind>> {
    Ok(match disk {
        Disk::Memory(len) => {
            LayeredDiskHandle::single_layer(RamDiskLayerHandle { len: Some(*len), sector_size: None }).into_resource()
        }
