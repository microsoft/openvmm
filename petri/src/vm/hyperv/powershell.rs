...
/// scripts (Register-NvmeEmulator.ps1, Add-NvmeEmulator.ps1,
    ...
    let dir = std::env::var(PETRI_NVME_EMULATOR_SCRIPTS_DIR).map_err(|_| {
        anyhow::anyhow!(
            "NVMe emulator scripts not found. Set {PETRI_NVME_EMULATOR_SCRIPTS_DIR} to the \
             ...
             (Register-NvmeEmulator.ps1, Add-NvmeEmulator.ps1, NvmeEmulatorCommon.psm1)."
        )
    })?;
    let dir = PathBuf::from(dir);
    for script in [
        "Register-NvmeEmulator.ps1",
        "Add-NvmeEmulator.ps1",
        "NvmeEmulatorCommon.psm1",
    ] {
...
/// Invokes Add-NvmeEmulator.ps1 to attach an NVMe emulator device
...
pub async fn run_add_nvme_emulator_device(
    ...
    let script = scripts_dir.join("Add-NvmeEmulator.ps1");
