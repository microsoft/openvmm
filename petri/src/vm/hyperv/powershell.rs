...
/// VTL encryption policies for Hyper-V VMs.
#[derive(Debug)]
pub enum HyperVManagementVtlEncryptionPolicy {
    /// Default encryption policy.
    Default = 0,
    /// Require GSP key encryption policy.
    RequireGspKey = 1,
    /// Forbid GSP key encryption policy.
    ForbidGspKey = 2,
    /// No encryption policy.
    None = 3,
    /// Hardware sealed secrets hash policy.
    HardwareSealedSecretsHashPolicy = 4,
    /// Hardware sealed secrets signer policy.
    HardwareSealedSecretsSignerPolicy = 5,
}

impl ps::AsVal for HyperVManagementVtlEncryptionPolicy {
    fn as_val(&self) -> impl '_ + AsRef<OsStr> {
        match self {
            HyperVManagementVtlEncryptionPolicy::Default => "0",
            HyperVManagementVtlEncryptionPolicy::RequireGspKey => "1",
            HyperVManagementVtlEncryptionPolicy::ForbidGspKey => "2",
            HyperVManagementVtlEncryptionPolicy::None => "3",
            HyperVManagementVtlEncryptionPolicy::HardwareSealedSecretsHashPolicy => "4",
            HyperVManagementVtlEncryptionPolicy::HardwareSealedSecretsSignerPolicy => "5",
        }
    }
}

/// Sets the management VTL encryption policy for a VM.
pub async fn run_set_management_vtl_encryption_policy(
    vmid: &Guid,
    ps_mod: &Path,
    policy: HyperVManagementVtlEncryptionPolicy,
) -> anyhow::Result<()> {
    tracing::trace!(?policy, ?vmid, "set management vtl encryption policy");
    ...
    run_host_cmd(
        PowerShellBuilder::new()
            ...
            .arg("Id", vmid)
            .pipeline()
            .cmdlet("Set-ManagementVtlEncryptionPolicy")
            .arg("Policy", policy)
            .finish()
            .build(),
    )
    .await
    .map(|_| ())
    .context("set_management_vtl_encryption_policy")
}
