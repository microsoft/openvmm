...
//! (`TEST_START` / `TEST_END`), accumulated in [`OpenTmkRun`].
...
fn apply_line(run: &mut OpenTmkRun, line: &str) -> LogTarget {
...
/// On timeout the partial [`OpenTmkRun`] has `ended == false`, which [`evaluate`]
/// reports as a hang.
pub async fn scan_opentmk_serial(
    ...
    let mut run = OpenTmkRun::default();
    let scan = async {
...
/// Connects to a Hyper-V guest's COM1 pipe and scans it for OpenTMK results.
///
/// Returns once the run completes or the internal safety timeout elapses; the
/// caller applies its own deadline while waiting for the returned task.
#[cfg(windows)]
pub(crate) async fn hyperv_opentmk_scan_task(
    ...
    loop {
            ...
            .open(&serial_pipe_path)
        {
            Ok(file) => {
                ...
                return scan_opentmk_serial(pipe, &log_file, SCAN_TIMEOUT).await;
            }
            Err(err) => {
                // Wait for the pipe, ignoring benign "not running yet"/"busy"
                // errors (see the Hyper-V serial log task).
...
/// Evaluates a completed [`OpenTmkRun`]; errors if the run did not pass.
///
/// A run passes only if the guest started, ended cleanly, did not panic, and
/// reported no failed assertions.
pub fn evaluate(run: &OpenTmkRun) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn run_lines(lines: &[&str]) -> OpenTmkRun {
        let mut run = OpenTmkRun::default();
        for line in lines {
