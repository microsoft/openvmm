...
//!
//! 1. **fio** ([`VirtioFsTest`]) — sequential/random read/write bandwidth
//!    (MiB/s) and IOPS against a large file on a virtio-fs mount.
//! 2. **readdir** ([`VirtioFsReaddirTest`]) — directory listing throughput
//!    (entries/s) for both plain READDIR and READDIRPLUS, exercising the FUSE
//!    readdir path in the VMM.
//!
//! Both boot a minimal Linux VM (linux_direct, pipette as PID 1) with a
//! virtio-fs device backed by a temporary host directory, and mount it
//! inside the guest.
//!
//! Layout inside the guest:
//!
//! ```text
//! /perf            <- erofs perf rootfs (read-only, contains fio + coreutils)
...
///
/// Creates a directory populated with many small files on the host side
/// and measures how long the guest takes to enumerate it. This directly
/// exercises the FUSE readdir / readdirplus paths in the VMM.
///
...
/// State kept across warm iterations for the readdir test.
...
impl crate::harness::WarmPerfTest for VirtioFsReaddirTest {
    ...
    async fn setup(
        ...
        let readdir_vfs_tag = "readdir_vfs";
        let readdir_vfs_tag_owned = readdir_vfs_tag.to_string();
            ...
            b.with_nic()
                ...
                .with_custom_config(move |c| {
                    ...
                    c.pcie_devices.push(PcieDeviceConfig {
                        ...
                        resource: virtio_resources::VirtioPciDeviceHandle(
                            virtio_resources::fs::VirtioFsHandle {
                                tag: readdir_vfs_tag_owned.clone(),
                                fs: virtio_resources::fs::VirtioFsBackend::HostFs {
                                    root_path: vfs_root_path,
                                    mount_options: String::new(),
                                },
        ...
        agent
            .mount(readdir_vfs_tag, "/perf/tmp/vfs", "virtiofs", 0, true)
            .await
