...
//!   (two NICs — Consomme for internet, TAP for iperf3; Linux only)
//!
...
pub enum NetBackend {
    ...
    Consomme,
    /// TAP device in a network namespace (two NICs, Linux only).
    Tap,
}
...
impl crate::harness::WarmPerfTest for NetworkTest {
    ...
    async fn setup(
        ...
        let (ready_send, ready_recv) =
            mesh::oneshot::<Result<crate::iperf_helper::IperfHelperReady, String>>();
        helper_mesh
            .launch_host(
...
mod tap {
    ...
    /// Configure guest networking for the TAP backend (two-NIC setup).
    ...
    pub(super) async fn setup_guest_networking(
        ...
        let find_consomme =
            "ip -o link show | grep -i 00:15:5d:12:12:12 | awk -F: '{print $2}' | tr -d ' '";
        let consomme_if = cmd!(sh, "sh -c {find_consomme}")
            .read()
            .await
            .context("failed to find Consomme NIC by MAC")?;
        let consomme_if = consomme_if.trim();
        let consomme_if = if consomme_if.is_empty() {
            "eth0"
        } else {
            consomme_if
        };
        tracing::info!(consomme_if, "detected Consomme NIC");
        ...
        let tap_if = cmd!(sh, "sh -c {find_tap}")
            .read()
            .await
            .context("failed to find TAP NIC by MAC")?;
        let tap_if = tap_if.trim();
        let tap_if = if tap_if.is_empty() { "eth1" } else { tap_if };
        tracing::info!(tap_if, "detected TAP NIC");

        cmd!(
            sh,
            "ifconfig {tap_if} 192.168.100.2 netmask 255.255.255.0 up"
        )
        .run()
        .await?;

        cmd!(sh, "ip route replace 192.168.100.0/24 dev {tap_if}")
            .run()
            .await?;
